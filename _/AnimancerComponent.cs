...
{
    /// <summary>
    /// The main component through which other scripts can interact with <see cref="AnimatorPro"/>. It allows you to play
    ...
    /// </remarks>
    /// https://website/animatorpro/api/AnimatorPro/AnimatorProComponent
    /// 
        ...
        IAnimancerComponent, IEnumerator, IAnimationClipSource, IAnimationClipCollection
    {
        ...
        /// </summary>
        public AnimatorProState Stop(AnimationClip clip) => Stop(GetKey(clip));

        /// <summary>
        /// Gets the state registered with the <see cref="IHasKey.Key"/>, stops and rewinds it to the start, then
        /// returns it.
        /// </summary>
        public AnimatorProState Stop(IHasKey hasKey) => _Playable?.Stop(hasKey);

        /// <summary>
        /// Gets the state associated with the `key`, stops and rewinds it to the start, then returns it.
        /// </summary>
        public AnimatorProState Stop(object key) => _Playable?.Stop(key);
