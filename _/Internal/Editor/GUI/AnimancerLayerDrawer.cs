...
namespace AnimatorPro.Editor
{
    ...
    /// </summary>
    /// https://website/animatorpro/api/AnimatorPro.Editor/AnimancerLayerDrawer
    /// 
    public sealed class AnimancerLayerDrawer : AnimancerNodeDrawer<AnimancerLayer>
    {
        ...
        public readonly List<AnimatorProState> ActiveStates = new List<AnimatorProState>();

        /// <summary>The states in the target layer which have zero <see cref="AnimancerNode.Weight"/>.</summary>
        public readonly List<AnimatorProState> InactiveStates = new List<AnimatorProState>();
        ...
        /// See <see cref="AnimancerPlayable.Play(AnimatorProState, float, FadeMode)"/>.
        /// </summary>
        private static void SortAndGatherKeys(List<AnimatorProState> states)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var keyState = key as AnimatorProState;
                if (keyState == null)
                    continue;
        ...
        private void DoStatesGUI(string label, List<AnimatorProState> states)
        {
        ...
        private readonly Dictionary<AnimatorProState, IAnimancerNodeDrawer>
            StateInspectors = new Dictionary<AnimatorProState, IAnimancerNodeDrawer>();

        /// <summary>Draws the Inspector for the given `state`.</summary>
        private void DoStateGUI(AnimatorProState state)
        {
        ...
        private void DoChildStatesGUI(AnimatorProState state)
        {
            EditorGUI.indentLevel++;
        ...
        private bool HasAnyStates(Func<AnimatorProState, bool> condition)
        {
