...
namespace AnimatorPro
{
    ...
    /// <para></para>
    /// Documentation: <see href="https://website/animatorpro/docs/manual/blending/layers">Layers</see>
    /// </remarks>
    /// https://website/animatorpro/api/AnimatorPro/AnimatorProLayer
    /// 
    public sealed class AnimancerLayer : AnimancerNode, IAnimationClipCollection
    {
        ...
        private readonly List<AnimatorProState> States = new List<AnimatorProState>();

        /************************************************************************************************************************/

        private AnimatorProState _CurrentState;
        ...
        /// on this layer. States controlled individually via methods in the <see cref="AnimatorProState"/> itself will
        /// not register in this property.
        /// <para></para>
        /// Each time this property changes, the <see cref="CommandCount"/> is incremented.
        /// </remarks>
        public AnimatorProState CurrentState
        {
            get => _CurrentState;
            private set
            {
        ...
        public override AnimatorProState GetChild(int index) => States[index];

        /// <summary>Returns the state connected to the specified `index` as a child of this layer.</summary>
        /// <remarks>This indexer is identical to <see cref="GetChild(int)"/>.</remarks>
        public AnimatorProState this[int index] => States[index];

        /************************************************************************************************************************/

        /// <summary>Adds a new port and uses <see cref="AnimatorProState.SetParent"/> to connect the `state` to it.</summary>
        public void AddChild(AnimatorProState state)
        {
        ...
        protected internal override void OnAddChild(AnimatorProState state) => OnAddChild(States, state);

        /************************************************************************************************************************/

        /// <summary>Disconnects the `state` from this layer at its <see cref="AnimancerNode.Index"/>.</summary>
        protected internal override void OnRemoveChild(AnimatorProState state)
        {
        ...
        /// <inheritdoc/>
        public override FastEnumerator<AnimatorProState> GetEnumerator()
            => new FastEnumerator<AnimatorProState>(States);
        ...
        /// <remarks>
        /// <see cref="AnimancerPlayable.GetKey"/> is used to determine the <see cref="AnimatorProState.Key"/>.
        /// </remarks>
        ...
        public AnimatorProState GetOrCreateState(AnimationClip clip, bool allowSetClip = false)
        {
        ...
        /// </summary>
        public AnimatorProState GetOrCreateState(ITransition transition)
        {
        ...
        /// </remarks>
        public AnimatorProState GetOrCreateState(object key, AnimationClip clip, bool allowSetClip = false)
        {
            if (key == null)
        ...
        /// <remarks>
        /// The animation will continue playing from its current <see cref="AnimatorProState.Time"/>.
        ...
        /// </remarks>
        public AnimatorProState Play(AnimationClip clip)
        ...
        /// </remarks>
        public AnimatorProState Play(AnimatorProState state)
        {
        ...
        /// and simply <see cref="AnimatorProState.Play"/> the `state`.
        /// <para></para>
        /// This method is safe to call repeatedly without checking whether the `state` was already playing.
        /// <para></para>
        /// <em>Animancer Lite only allows the default `fadeDuration` (0.25 seconds) in runtime builds.</em>
        /// </remarks>
        public AnimatorProState Play(AnimationClip clip, float fadeDuration, FadeMode mode = default)
        ...
        /// <em>Animancer Lite only allows the default `fadeDuration` (0.25 seconds) in runtime builds.</em>
        /// </remarks>
        public AnimatorProState Play(AnimatorProState state, float fadeDuration, FadeMode mode = default)
        {
            // Skip the fade if:
        ...
        /// <see cref="Play(AnimatorProState)"/> or <see cref="Play(AnimatorProState, float, FadeMode)"/>
        ...
        /// </remarks>
        public AnimatorProState Play(ITransition transition)
        ...
        /// This method is safe to call repeatedly without checking whether the `transition` was already playing.
        /// </remarks>
        public AnimatorProState Play(ITransition transition, float fadeDuration, FadeMode mode = default)
        {
            var state = Root.States.GetOrCreate(transition);
        ...
        /// </remarks>
        public AnimatorProState TryPlay(object key)
        ...
        /// <em>Animancer Lite only allows the default `fadeDuration` (0.25 seconds) in runtime builds.</em>
        /// </remarks>
        public AnimatorProState TryPlay(object key, float fadeDuration, FadeMode mode = default)
        ...
        /// <exception cref="ArgumentException">
        /// The <see cref="AnimatorProState.Clip"/> is null when using <see cref="FadeMode.FromStart"/> or
        /// <see cref="FadeMode.NormalizedFromStart"/>.
        /// </exception>
        private void EvaluateFadeMode(FadeMode mode, ref AnimatorProState state, ref float fadeDuration)
        {
            switch (mode)
            {
        ...
        /// </summary>
        /// <exception cref="InvalidOperationException">The <see cref="AnimatorProState.Clip"/> is null.</exception>
        /// <exception cref="ArgumentOutOfRangeException">
        /// More states have been created for this <see cref="AnimatorProState.Clip"/> than the
        /// <see cref="MaxStateDepth"/> allows.
        /// </exception>
        public AnimatorProState GetOrCreateWeightlessState(AnimatorProState state)
        {
            if (state.Weight != 0)
            {
                var clip = state.Clip;
                ...
                while (true)
                {
                    keyState = keyState.Key as AnimatorProState;
                    if (keyState == null)
                    {
                        break;
                    }
        ...
        /// <summary>
        /// Sets <see cref="AnimancerNode.Weight"/> = 0 and calls <see cref="AnimatorProState.Stop"/> on all animations
